# LOCK-DDL-PREFLIGHT-CHECK-LONG-TXN
...
## 来源依据摘要
执行强 DDL 前应检查长事务、idle in transaction 和目标表锁，避免 ACCESS EXCLUSIVE 阻塞事故。
...
## 标记理由
P0 测试点；用于覆盖本地知识补充 13.8.3 Maintenance And Ddl Lock Practices 中的 执行强 DDL 前应检查长事务、idle in transaction 和目标表锁，避免 ACCESS EXCLUSIVE 阻塞事故。

## 备注
来源：docs/archive/concurrency-control-by-official-chapter/13.8-lock-knowledge-extensions/13.8.3-maintenance-and-ddl-lock-practices/lock-ddl-preflight-check-long-txn.md