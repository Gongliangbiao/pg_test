# TCL-SET-SNAPSHOT-SERIALIZABLE-COMPATIBILITY

## 来源声明
- source_type: official-doc
- source_ref: PostgreSQL 16 Chapter 74 Transaction Processing
- source_note: 来源于 PostgreSQL 官方文档 Chapter 74 的测试点计划。


## 官方章节
附录 A

## 官方依据摘要
导入方若为 SERIALIZABLE，导出 snapshot 的事务也必须为 SERIALIZABLE；非只读 serializable 事务不能从只读事务导入 snapshot。

## 来源因子
- factor_matrix: docs/test-factors/chapter-74-transaction-processing/appendix-a-tcl-command-boundaries/factor-matrix.md
- factor_values: F01-V18,F02-V07,F03-V05,F04-V02
- combination_strategy: single-factor

## 测试点
导入方若为 SERIALIZABLE，导出 snapshot 的事务也必须为 SERIALIZABLE；非只读 serializable 事务不能从只读事务导入 snapshot。

## 覆盖类型
正向

## 重要边界
- 被测对象/语句：snapshot。
- 触发状态/条件：只读事务状态。
- 边界或异常：常规核心路径。
- 本测试点只抽取事务处理机制的覆盖目标，不包含 SQL 执行步骤。

## 测试必要性
supporting

## 标记理由
P1 测试点；来自 Chapter 74 Transaction Processing 计划。

## 备注
来源：docs/plans/transaction-processing/test-point-plan.md