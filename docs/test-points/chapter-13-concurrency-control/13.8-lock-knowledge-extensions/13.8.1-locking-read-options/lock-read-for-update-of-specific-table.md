# LOCK-READ-FOR-UPDATE-OF-SPECIFIC-TABLE
...
## 来源依据摘要
FOR UPDATE OF table_name 在多表 join 中只锁定指定表的目标行，不锁其他 join 表行。
...
## 标记理由
P0 测试点；用于覆盖本地知识补充 13.8.1 Locking Read Options 中的 FOR UPDATE OF table_name 在多表 join 中只锁定指定表的目标行，不锁其他 join 表行。

## 备注
来源：docs/archive/concurrency-control-by-official-chapter/13.8-lock-knowledge-extensions/13.8.1-locking-read-options/lock-read-for-update-of-specific-table.md