...
\! sh commonScript/del_sync_points.sh txid_vxid_local_sequence_per_backend
BEGIN;
SELECT virtualtransaction AS s1_virtualxid
FROM pg_locks WHERE pid = pg_backend_pid() AND locktype = 'virtualxid'
ORDER BY virtualtransaction LIMIT 1;
\! sh commonScript/set_sync_point.sh txid_vxid_local_sequence_per_backend_s1_to_s2_01
\! sh commonScript/get_sync_point.sh txid_vxid_local_sequence_per_backend_s2_to_s1_01
COMMIT;
