...
## 来源
- source_test_point: docs/test-points/chapter-13-concurrency-control/13.1-introduction/mvcc-read-write-nonblocking-select-update.md
...
## 表结构设计
- 表名使用 tab_131_mvcc_read_write_nonblocking_select_update，列类型按 MVCC 场景选择文本、数值或时间戳字段。

## 测试数据
- 插入单行确定性数据，用于观察提交前后版本变化。

## 执行设计
S1 在事务中执行普通 SELECT；S2 并发 UPDATE 同一行并提交；验证普通读不阻塞写，且 S1 后续新语句在 READ COMMITTED 下可见提交结果。

## 预期结果
- S2 UPDATE 能完成。
- S1 后续 SELECT 看到 S2 提交结果。
