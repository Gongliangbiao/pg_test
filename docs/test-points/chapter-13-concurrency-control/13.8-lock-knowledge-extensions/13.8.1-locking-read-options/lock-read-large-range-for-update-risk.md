# LOCK-READ-LARGE-RANGE-FOR-UPDATE-RISK
...
## 来源依据摘要
大范围 FOR UPDATE 会锁住大量行，增加阻塞、死锁和 tuple 标记写入成本，应作为风险场景记录。
...
## 标记理由
P0 测试点；用于覆盖本地知识补充 13.8.1 Locking Read Options 中的 大范围 FOR UPDATE 会锁住大量行，增加阻塞、死锁和 tuple 标记写入成本，应作为风险场景记录。

## 备注
来源：docs/archive/concurrency-control-by-official-chapter/13.8-lock-knowledge-extensions/13.8.1-locking-read-options/lock-read-large-range-for-update-risk.md